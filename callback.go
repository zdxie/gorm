...
func (cp *CallbackProcessor) Register(callbackName string, callback func(scope *Scope)) {
	if cp.kind == "row_query" {
		if cp.before == "" && cp.after == "" && callbackName != "gorm:row_query" {
			cp.logger.Print(context.TODO(), "info", fmt.Sprintf("Registering RowQuery callback %v without specify order with Before(), After(), applying Before('gorm:row_query') by default for compatibility...", callbackName))
			cp.before = "gorm:row_query"
		}
	}

	cp.logger.Print(context.TODO(), "info", fmt.Sprintf("[info] registering callback `%v` from %v", callbackName, fileWithLineNum()))
...
func (cp *CallbackProcessor) Remove(callbackName string) {
	cp.logger.Print(context.TODO(), "info", fmt.Sprintf("[info] removing callback `%v` from %v", callbackName, fileWithLineNum()))
	cp.name = callbackName
	cp.remove = true
	cp.parent.processors = append(cp.parent.processors, cp)
	cp.parent.reorder()
...
func (cp *CallbackProcessor) Replace(callbackName string, callback func(scope *Scope)) {
	cp.logger.Print(context.TODO(), "info", fmt.Sprintf("[info] replacing callback `%v` from %v", callbackName, fileWithLineNum()))
	cp.name = callbackName
	cp.processor = &callback
	cp.replace = true
	cp.parent.processors = append(cp.parent.processors, cp)
...
func sortProcessors(cps []*CallbackProcessor) []*func(scope *Scope) {
	...
	for _, cp := range cps {
		...
		if index := getRIndex(allNames, cp.name); index > -1 && !cp.replace && !cp.remove {
			cp.logger.Print(context.TODO(), "warning", fmt.Sprintf("[warning] duplicated callback `%v` from %v", cp.name, fileWithLineNum()))
		}
